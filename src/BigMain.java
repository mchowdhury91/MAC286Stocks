...
public class BigMain {
	...
	public static void main(String[] args) throws IOException {
		...
		if (directory.exists() && directory.isDirectory()) {
			...
			int[] dayLimits = { 5, 10, 15 };
			

			
			CustomSimulator cSimulator = new CustomSimulator();
			Simulator simulator = cSimulator;

			for (int dayLimit : dayLimits) {
				Stats[] statList = new Stats[stopLossValues.length * targetValues.length];
				int listIndex = 0;
				
				cSimulator.setDaysToSellAfter(dayLimit);
				File statsDir = new File("Stats for dL " + dayLimit);
				if (!statsDir.exists()) {
					statsDir.mkdir();
				}

				File tradesDir = new File(statsDir + "/Trades for dL " + dayLimit);
				if (!tradesDir.exists()) {
					tradesDir.mkdir();
				}
				...
				BufferedWriter bW2 = new BufferedWriter(new FileWriter(statsDir.getName() + "/" + "Stats for dL " + dayLimit + ".csv"));
				String header = "";
